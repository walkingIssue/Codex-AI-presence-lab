    ...
    def test_renderer_template_applies_full_state_in_cubism_lifecycle(self) -> None:
        ...
        harness = textwrap.dedent(
            ...
            setImmediate(() => setImmediate(() => {
              ...
              if (typeof callbacks.curation !== "function") throw new Error("renderer did not subscribe to profile curation");
              callbacks.curation({
                schema: "codex-ai-presence/profile-curation/v0.1",
                initial_actions: ["toggle.overlay"],
                activity_actions: { thinking: { add: [], suppress: [] } },
              });
              ...
              if (values.get("control.base") !== 0 || values.get("control.overlay") !== 1) {
                throw new Error("profile initial actions did not replace the parent default");
              }
              callbacks.avatar({ avatar_id: "demo-avatar", revision: 0, actions: [] });
              values.set("control.base", 0); values.set("control.overlay", 0); calls.length = 0;
              lifecycle.beforeModelUpdate();
              if (values.get("control.base") !== 0 || values.get("control.overlay") !== 0) {
                throw new Error("routed state did not replace profile initial actions");
              }
              callbacks.audio({ type: "activity", state: "thinking", ttl_ms: 500 });
              lifecycle.beforeModelUpdate();
              if (calls.includes("control.overlay")) {
                throw new Error("profile activity override did not clear the parent add rule");
              }
              ...
              callbacks.curation({ schema: "codex-ai-presence/profile-curation/v0.1", initial_actions: [] });
              values.set("control.base", 0); values.set("control.overlay", 0); calls.length = 0;
              lifecycle.beforeModelUpdate();
              if (values.get("control.base") !== 1 || values.get("control.overlay") !== 1) {
                throw new Error("profile curation overrode an explicit routed state");
              }
              ...
              process.stdout.write(JSON.stringify({ lifecycle: true, responsiveResize: true, haloConfig: true, profileCuration: true, childActivityOverride: true, routedStateAuthority: true, activityOverlay: true, activityExpiry: true, activitySuppress: true, reset: true, replacement: true, replayOrder: true, rigMotion: true, eyelids: true, jaw: true }));
            }));
            """
        )
        ...
        self.assertEqual(
            ...
            {"lifecycle": True, "responsiveResize": True, "haloConfig": True, "profileCuration": True, "childActivityOverride": True, "routedStateAuthority": True, "activityOverlay": True, "activityExpiry": True, "activitySuppress": True, "reset": True, "replacement": True, "replayOrder": True, "rigMotion": True, "eyelids": True, "jaw": True},
        )
