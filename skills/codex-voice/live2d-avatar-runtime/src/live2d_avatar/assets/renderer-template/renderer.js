(() => {
  ...
  let profileCuration = null;
  let hasRoutedAvatarState = false;
  ...
  function resolvedActivityRule(nextActivity) {
    const parent = activityRule(activityActions[nextActivity]);
    const childRules = profileCuration?.activity_actions;
    const child = childRules && typeof childRules === "object" ? childRules[nextActivity] : null;
    if (!child || typeof child !== "object") return parent;
    return {
      add: Object.prototype.hasOwnProperty.call(child, "add") ? knownActionIds(child.add) : parent.add,
      suppress: Object.prototype.hasOwnProperty.call(child, "suppress")
        ? knownActionIds(child.suppress)
        : parent.suppress,
    };
  }

  function refreshActivityActions() {
    const rule = resolvedActivityRule(activity);
    activityActionIds = new Set(rule.add);
    suppressedActionIds = new Set(rule.suppress);
  }

  function normalizeProfileCuration(payload) {
    if (!payload || typeof payload !== "object" || payload.schema !== PROFILE_CURATION_SCHEMA) return null;
    const normalized = {};
    if (Object.prototype.hasOwnProperty.call(payload, "initial_actions")) {
      if (!Array.isArray(payload.initial_actions)) return null;
      normalized.initial_actions = knownActionIds(payload.initial_actions);
    }
    if (Object.prototype.hasOwnProperty.call(payload, "activity_actions")) {
      const rawActivity = payload.activity_actions;
      if (!rawActivity || typeof rawActivity !== "object" || Array.isArray(rawActivity)) return null;
      const activityActions = {};
      for (const [state, rawRule] of Object.entries(rawActivity)) {
        if (!Object.prototype.hasOwnProperty.call(colors, state)
          || !rawRule || typeof rawRule !== "object" || Array.isArray(rawRule)) return null;
        if (Object.keys(rawRule).some((field) => field !== "add" && field !== "suppress")) return null;
        const rule = {};
        if (Object.prototype.hasOwnProperty.call(rawRule, "add")) {
          if (!Array.isArray(rawRule.add)) return null;
          rule.add = knownActionIds(rawRule.add);
        }
        if (Object.prototype.hasOwnProperty.call(rawRule, "suppress")) {
          if (!Array.isArray(rawRule.suppress)) return null;
          rule.suppress = knownActionIds(rawRule.suppress);
        }
  ...
  function applyProfileCuration(payload) {
    const nextCuration = normalizeProfileCuration(payload);
    if (!nextCuration) return;
    profileCuration = nextCuration;
    if (!hasRoutedAvatarState && Object.prototype.hasOwnProperty.call(profileCuration, "initial_actions")) {
      activeActionIds = new Set(profileCuration.initial_actions);
    }
  ...
  function attachOrbBridge() {
    ...
    window.addEventListener("mousemove", (event) => {
      const wantsMove = modifierHeld(event);
    ...
    avatar.addEventListener("pointerdown", (event) => {
      if (!moveMode || event.button !== 0 || !modifierHeld(event)) return;
      dragging = true;
    ...
    avatar.addEventListener("pointermove", (event) => {
      if (moveMode && dragging) window.orbApi.drag({ screenX: event.screenX, screenY: event.screenY });
    });
