...
(function(global, factory) {
  ...
  class CubismMatrix44 {
    ...
    translateRelative(x, y) {
      this._tr[12] += x * this._tr[0] + y * this._tr[4];
      this._tr[13] += x * this._tr[1] + y * this._tr[5];
      this._tr[14] += x * this._tr[2] + y * this._tr[6];
      this._tr[15] += x * this._tr[3] + y * this._tr[7];
    }
    translate(x, y) {
      this._tr[12] = x;
      this._tr[13] = y;
  ...
  class CubismRenderer {
    ...
    getModelColor() {
      this._drawColor.R = this._modelColor.R;
      this._drawColor.G = this._modelColor.G;
      this._drawColor.B = this._modelColor.B;
      this._drawColor.A = this._modelColor.A;
    ...
    constructor() {
      ...
      this._drawColor = new CubismTextureColor();
  ...
  class CubismModel {
    ...
    saveParameters() {
      ...
      if (!(this._savedParameters instanceof Float32Array) || this._savedParameters.length !== parameterCount) {
        this._savedParameters = new Float32Array(parameterCount);
      }
      this._savedParameters.set(this._parameterValues.subarray(0, parameterCount));
    }
    getModel() {
      return this._model;
    }
    getPartIndex(partId) {
      const cachedIndex = this._partIndices.get(partId);
      if (cachedIndex !== void 0) {
        return cachedIndex;
      }
      ...
      const partIndex = partCount + this._notExistPartCount++;
      this._notExistPartId[partId] = partIndex;
      this._partIndices.set(partId, partIndex);
    ...
    getParameterIndex(parameterId) {
      const cachedIndex = this._parameterIndices.get(parameterId);
      if (cachedIndex !== void 0) {
        return cachedIndex;
      }
      ...
      const parameterIndex = this._model.parameters.count + this._notExistParameterCount++;
      this._notExistParameterId[parameterId] = parameterIndex;
      this._parameterIndices.set(parameterId, parameterIndex);
    ...
    getDrawableIndex(drawableId) {
      const drawableIndex = this._drawableIndices.get(drawableId);
      return drawableIndex === void 0 ? -1 : drawableIndex;
    }
    getDrawableCount() {
    ...
    loadParameters() {
      ...
      this._parameterValues.set(this._savedParameters.subarray(0, parameterCount), 0);
    }
    initialize() {
      ...
      this._parameterMinimumValues = this._model.parameters.minimumValues;
      {
        ...
        for (let i = 0; i < parameterCount; ++i) {
          ...
          this._parameterIds.push(parameterId);
          this._parameterIndices.set(parameterId, i);
        }
      }
      {
        ...
        for (let i = 0; i < partCount; ++i) {
          ...
          this._partIds.push(partId);
          this._partIndices.set(partId, i);
        }
      }
      {
        ...
        for (let i = 0; i < drawableCount; ++i) {
          ...
          this._drawableIds.push(drawableId);
          this._drawableIndices.set(drawableId, i);
        }
      }
    }
    constructor(model) {
      this._model = model;
      this._savedParameters = new Float32Array(0);
      this._parameterIds = [];
      this._drawableIds = [];
      this._partIds = [];
      this._parameterIndices = /* @__PURE__ */ new Map();
      this._drawableIndices = /* @__PURE__ */ new Map();
      this._partIndices = /* @__PURE__ */ new Map();
      this._notExistPartId = {};
      this._notExistParameterId = {};
      this._notExistParameterValues = {};
      this._notExistPartOpacities = {};
      this._notExistPartCount = 0;
      this._notExistParameterCount = 0;
      this.initialize();
    }
    release() {
      this._model.release();
  ...
  function bezierEvaluate(points, time) {
    ...
    const p01 = points[0].value + (points[1].value - points[0].value) * t;
    const p12 = points[1].value + (points[2].value - points[1].value) * t;
    const p23 = points[2].value + (points[3].value - points[2].value) * t;
  ...
  function evaluateCurve(motionData, index, time) {
    ...
    return segment.evaluate(segment.points, time);
  }
    ...
    parse(motionJson) {
      ...
      for (let curveCount = 0; curveCount < this._motionData.curveCount; ++curveCount) {
        ...
        for (let segmentPosition = 0; segmentPosition < json.getMotionCurveSegmentCount(curveCount); ) {
          ...
          const parsedSegment = this._motionData.segments[totalSegmentCount];
          const segmentPointCount = parsedSegment.segmentType == CubismMotionSegmentType.CubismMotionSegmentType_Bezier ? 4 : 2;
          parsedSegment.points = this._motionData.points.slice(parsedSegment.basePointIndex, parsedSegment.basePointIndex + segmentPointCount);
          ++curve.segmentCount;
          ++totalSegmentCount;
        }
  ...
  class CubismPhysicsJson {
    ...
    getFps() {
      const fps = Number(this._json.Meta.Fps);
      return Number.isFinite(fps) && fps > 0 ? fps : MaximumPhysicsFps;
    }
  ...
  class CubismPhysics {
    ...
    evaluate(model, deltaTimeSeconds) {
      const interval = this._physicsInterval;
      const finiteDelta = Number.isFinite(deltaTimeSeconds) && deltaTimeSeconds > 0
        ? deltaTimeSeconds
        : interval;
      const boundedDelta = Math.min(finiteDelta, interval * 2);
      if (!this._physicsInitialized) {
        this._step(model, Math.min(boundedDelta, interval), true);
        this._physicsInitialized = true;
        this._applyOutputs(model, 1);
        return;
      }
      this._physicsAccumulator = Math.min(this._physicsAccumulator + boundedDelta, interval * 2);
      while (this._physicsAccumulator >= interval) {
        this._step(model, interval, false);
        this._physicsAccumulator -= interval;
      }
      this._applyOutputs(model, this._physicsAccumulator / interval);
    }
    _step(model, deltaTimeSeconds, initializeOutputs) {
      let weight;
      let radAngle;
      let outputValue;
      const totalAngle = this._totalAngle;
      const totalTranslation = this._totalTranslation;
      const translation = this._outputTranslation;
      let currentSetting;
      let currentInput;
      let currentOutput;
      ...
      for (let settingIndex = 0; settingIndex < this._physicsRig.subRigCount; ++settingIndex) {
        ...
        currentInput = currentSetting.inputs;
        currentOutput = currentSetting.outputs;
        currentParticles = currentSetting.particles;
        ...
        for (let i = 0; i < currentSetting.outputCount; ++i) {
          ...
          currentOutput[i].previousValue = initializeOutputs ? outputValue : currentOutput[i].currentValue;
          currentOutput[i].currentValue = outputValue;
        }
      }
    }
    _applyOutputs(model, interpolationWeight) {
      const coreParameters = model.getModel().parameters;
      const parameterValue = coreParameters.values;
      const parameterMaximumValue = coreParameters.maximumValues;
      const parameterMinimumValue = coreParameters.minimumValues;
      const outputs = this._physicsRig.outputs;
      for (let i = 0; i < outputs.length; ++i) {
        const output = outputs[i];
        if (output.destinationParameterIndex == -1) {
          output.destinationParameterIndex = model.getParameterIndex(output.destination.id);
        }
        const destinationParameterIndex = output.destinationParameterIndex;
        const outputValue = output.previousValue
          + (output.currentValue - output.previousValue) * interpolationWeight;
    ...
    constructor() {
      ...
      this._totalAngle = { angle: 0 };
      this._totalTranslation = new CubismVector2(0, 0);
      this._outputTranslation = new CubismVector2(0, 0);
      this._physicsAccumulator = 0;
      this._physicsInterval = 1 / MaximumPhysicsFps;
    ...
    parse(physicsJson) {
      ...
      this._physicsInterval = 1 / Math.min(MaximumPhysicsFps, json.getFps());
      ...
      for (let i = 0; i < this._physicsRig.subRigCount; ++i) {
        ...
        setting.inputs = this._physicsRig.inputs.slice(setting.baseInputIndex, setting.baseInputIndex + setting.inputCount);
        setting.outputs = this._physicsRig.outputs.slice(setting.baseOutputIndex, setting.baseOutputIndex + setting.outputCount);
        setting.particles = this._physicsRig.particles.slice(setting.baseParticleIndex, setting.baseParticleIndex + setting.particleCount);
    ...
    initialize() {
      ...
      for (let settingIndex = 0; settingIndex < this._physicsRig.subRigCount; ++settingIndex) {
        currentSetting = this._physicsRig.settings[settingIndex];
        strand = currentSetting.particles;
  ...
  function getOutputAngle(translation, particles, particleIndex, isInverted, parentGravity) {
    ...
    let parentY;
    if (particleIndex >= 2) {
      parentX = particles[particleIndex - 1].position.x - particles[particleIndex - 2].position.x;
      parentY = particles[particleIndex - 1].position.y - particles[particleIndex - 2].position.y;
    } else {
      parentX = -parentGravity.x;
      parentY = -parentGravity.y;
    }
    let outputValue = Math.atan2(translation.y, translation.x) - Math.atan2(parentY, parentX);
    while (outputValue < -Math.PI) outputValue += Math.PI * 2;
    while (outputValue > Math.PI) outputValue -= Math.PI * 2;
    if (isInverted) {
      outputValue *= -1;
    }
  ...
  function updateParticles(strand, strandCount, totalTranslation, totalAngle, windDirection, thresholdValue, deltaTimeSeconds, airResistance) {
    ...
    let currentGravityX = Math.sin(totalRadian);
    let currentGravityY = Math.cos(totalRadian);
    const gravityLength = Math.sqrt(currentGravityX * currentGravityX + currentGravityY * currentGravityY);
    currentGravityX /= gravityLength;
    currentGravityY /= gravityLength;
    for (let i = 1; i < strandCount; ++i) {
      const particle = strand[i];
      const parent = strand[i - 1];
      const forceX = currentGravityX * particle.acceleration + windDirection.x;
      const forceY = currentGravityY * particle.acceleration + windDirection.y;
      particle.force.x = forceX;
      particle.force.y = forceY;
      particle.lastPosition.x = particle.position.x;
      particle.lastPosition.y = particle.position.y;
      const delay = particle.delay * deltaTimeSeconds * 30;
      let directionX = particle.position.x - parent.position.x;
      let directionY = particle.position.y - parent.position.y;
      let radian = Math.atan2(currentGravityY, currentGravityX)
        - Math.atan2(particle.lastGravity.y, particle.lastGravity.x);
      while (radian < -Math.PI) radian += Math.PI * 2;
      while (radian > Math.PI) radian -= Math.PI * 2;
      radian /= airResistance;
      const cosine = Math.cos(radian);
      const sine = Math.sin(radian);
      directionX = cosine * directionX - directionY * sine;
      directionY = sine * directionX + directionY * cosine;
      particle.position.x = parent.position.x + directionX
        + particle.velocity.x * delay + forceX * delay * delay;
      particle.position.y = parent.position.y + directionY
        + particle.velocity.y * delay + forceY * delay * delay;
      let normalizedX = particle.position.x - parent.position.x;
      let normalizedY = particle.position.y - parent.position.y;
      const directionLength = Math.sqrt(normalizedX * normalizedX + normalizedY * normalizedY);
      normalizedX /= directionLength;
      normalizedY /= directionLength;
      particle.position.x = parent.position.x + normalizedX * particle.radius;
      particle.position.y = parent.position.y + normalizedY * particle.radius;
      if (Math.abs(particle.position.x) < thresholdValue) {
        particle.position.x = 0;
      }
      if (delay != 0) {
        particle.velocity.x = (particle.position.x - particle.lastPosition.x) / delay * particle.mobility;
        particle.velocity.y = (particle.position.y - particle.lastPosition.y) / delay * particle.mobility;
      }
      particle.force.x = 0;
      particle.force.y = 0;
      particle.lastGravity.x = currentGravityX;
      particle.lastGravity.y = currentGravityY;
    }
  }
  function updateOutputParameterValue(parameterValues, parameterIndex, parameterValueMinimum, parameterValueMaximum, translation, output) {
    let outputScale;
    let value;
    let weight;
    ...
    if (weight >= 1) {
      ...
      value = parameterValues[parameterIndex] * (1 - weight) + value * weight;
      parameterValues[parameterIndex] = value;
    }
  }
  ...
  class CubismClippingManager_WebGL {
    ...
    setupLayoutBounds(usingClipCount) {
      ...
      for (let channelNo = 0; channelNo < ColorChannelCount; channelNo++) {
        ...
        } else if (exports2.CubismConfig.supportMoreMaskDivisions && layoutCount <= 25) {
          const gridSize = layoutCount <= 16 ? 4 : 5;
          for (let i = 0; i < layoutCount; i++) {
            let xpos = i % gridSize;
            let ypos = i / gridSize;
            xpos = ~~xpos;
  ...
  class CubismRenderer_WebGL extends CubismRenderer {
    ...
    release() {
      ...
      if (this.gl != null) {
        if (this._bufferData.vertex != null) this.gl.deleteBuffer(this._bufferData.vertex);
        if (this._bufferData.uv != null) this.gl.deleteBuffer(this._bufferData.uv);
        if (this._bufferData.index != null) this.gl.deleteBuffer(this._bufferData.index);
      }
...
`, e);
  ...
  class Cubism4InternalModel extends InternalModel {
    ...
    updateFocus() {
      const x = this.focusController.x;
      const y = this.focusController.y;
      if (x === 0 && y === 0) return;
      this.coreModel.addParameterValueById(this.idParamEyeBallX, x);
      this.coreModel.addParameterValueById(this.idParamEyeBallY, y);
      this.coreModel.addParameterValueById(this.idParamAngleX, x * 30);
      this.coreModel.addParameterValueById(this.idParamAngleY, y * 30);
      this.coreModel.addParameterValueById(this.idParamAngleZ, x * y * -30);
      this.coreModel.addParameterValueById(this.idParamBodyAngleX, x * 10);
    }
    ...
    draw(gl) {
      ...
      // Avatar models render directly into their dedicated transparent canvas.
      // MSAA is disabled by the host, so the target framebuffer is deterministically
      // the default framebuffer and a synchronous gl.getParameter call is avoidable.
      this.renderer.setRenderState(null, this.viewport);
