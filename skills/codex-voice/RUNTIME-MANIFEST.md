...
| `2026-07-14-fedora-openvino-kokoro` | Uses Kokoro's FP16 GPU export for Intel Arc and generates an OpenVINO-specific graph that flattens unsupported rank-3 interpolation to rank 2 and replaces the unranked STFT boundary with equivalent DFT convolution kernels; HETERO routing keeps GPU-compatible partitions on Arc with CPU fallback for remaining seams | The source model remains under `.codex-voice`; the generated model remains in the existing owned `gpu_patch/` boundary |
