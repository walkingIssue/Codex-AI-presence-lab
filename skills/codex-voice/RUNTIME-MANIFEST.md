...
| `2026-07-14-fedora-openvino-kokoro` | Splits Kokoro's unquantized export at its single ALBERT output; native OpenVINO runs only the FP32 text encoder on Intel Arc, while an untouched ONNX Runtime CPU graph retains duration, prosody, STFT, and waveform synthesis | The source model remains under `.codex-voice`; both generated split models remain in the existing owned `gpu_patch/` boundary |
