...
| `skills/codex-voice/live2d-avatar-runtime/` | unified skill projection | codex-voice skill payload | skill reinstall/reprojection | implemented |
| `skills/codex-voice/scripts/live2d-avatar.{py,sh,ps1}` | unified skill projection | codex-voice skill payload | skill reinstall/reprojection | implemented |
...
- The runtime currently starts no process, opens no port, and writes no file inside `.codex-voice` directly. It invokes the Voice-owned avatar writer through the documented bridge.
...
5. If the unified skill installation changes, update `skills/codex-voice/RUNTIME-MANIFEST.md` and re-project it from the repository source. Do not create a second `live2d-avatar-controls` skill installation.
